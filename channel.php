...
?>
	...
	<table width="50%" cellpadding="0" cellspacing="0" align="right">
		...
		$flist=$f1['mediaid'];
	    $ss="select cname from subscription where cname='$username[$y]' and username='$uname'";
	    $out=$out= mysql_query($ss);
		$s1 = mysql_fetch_assoc($out);
		$slist=$s1['cname'];		
...
?><tr valign="top" >
	<form method='get' action=''><?php
		if ($y==1) {?>
			...
			<th><?php echo $username[$y];?></th><td></td><td></td><td></td><td></td>
			<td>
				
						  <?php if ($username[$y]==$slist){  ?>

								<button class="btn btn-success" name=" 'subscribe'<?php echo$y;?>" type="submit" value="submit" disabled>subscribed</button>
								<?php

								} 
							else {?>
							<button class="btn btn-success" name=" 'subscribe'<?php echo$y;?>" type="submit" value="submit">subscribe</button>
						  <?php } ?>
			</td>
		...
		else if ($username[$y]!=$username[$y-1]&&$y>1)
		{ 
		?>
			<th><?php echo $username[$y]?></th><td></td><td></td><td></td><td></td>
			<td> 
...
?>
	...
	<?php } //}?>
			...
			$output4 = mysql_query( $insert3);
			   ...
			   for ($r=1;$r<=$y;$r++){
	if(isset($_GET["'subscribe'$r"]))
			{
				echo "inside get";
			
  			$insert7 = "insert into subscription(cname, username)".
							  "values('$username[$r]','$uname')";
			$output7 = mysql_query( $insert7);
			if($output7){
				header("Location: channel.php");
			}
			else{
