...
?>
	...
	<table width="50%" cellpadding="0" cellspacing="0" align="right">
			...
			<th><?php echo $username[$y];?></th><td></td><td></td><td></td><td></td>
			 <td>
						  <?php if ($username[$y]==$slist){  ?>
								...
								<button class="btn btn-success" name=" 'sub'<?php echo$y;?>" type="submit" value="submit">unsubscribe</button>
								<?php
		...
		else if ($y>1&&$username[$y]!=$username[$y-1]&&$username[$y]==$slist)
		{
		?>
		...
		if(isset($_GET["'sub'$w"]))
			{
  			...
  			$delete7 = "delete from subscription where cname='$username[$w]' and username='$uname'";
			$output7 = mysql_query( $delete7);
			if($output7){
				header("Location: subs.php");
			}
			else{
